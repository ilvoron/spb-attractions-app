...
export const EditAttractionPage = () => {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            // Обновляем кэш для достопримечательности и списка
            queryClient.invalidateQueries(['attraction', id]);
            queryClient.invalidateQueries(['attractions']);
                    ...
                    `}
                                        >
                                            ...
                                            <div className="flex justify-between items-center">
                                                ...
                                                >
                                                    {image.originalName && image.originalName.length > 15
                                                        ? image.originalName.substring(0, 12) + '...'
                                                        : image.originalName}
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                                        {newImages.map((image, index) => (
                                            <div
                                                ...
                                                className={`
                        relative p-2 rounded-lg border-2 
                        ${index === primaryNewImageIndex ? 'border-blue-500 bg-blue-50' : 'border-gray-200'}
                    `}
                                            >
                                                {/* Предпросмотр изображения */}
                                                <div className="w-full h-32 bg-gray-100 rounded overflow-hidden mb-2">
                                                ...
                                                <div className="flex justify-between items-center">
                                                    <div className="text-xs text-gray-500 truncate" title={image.name}>
